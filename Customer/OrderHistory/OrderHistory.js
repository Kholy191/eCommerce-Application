document.addEventListener("DOMContentLoaded", function () {
  ...
  // Fetch order history (only "Drafted" status)
...
async function loadOrderHistory(customerId) {
  try {
    ...
    if (
      !customerCart ||
      !customerCart.orders ||
      customerCart.orders.length === 0
    ) {
      orderHistoryList.innerHTML =
        '<div class="no-orders">You have no draft orders yet.</div>';
      return;
    }

    // Filter only "Drafted" orders
    const draftedOrders = customerCart.orders.filter(
      (order) => order.status === "Drafted"
    );

    if (draftedOrders.length === 0) {
      orderHistoryList.innerHTML =
        '<div class="no-orders">You have no draft orders.</div>';
      return;
    }
    ...
    totalElement.innerHTML = `<strong>Grand Total: $${totalAmount.toFixed(
      2
    )}</strong>`;
    ...
    orderHistoryList.innerHTML = `
        <div class="no-orders">
          Error loading your draft orders. Please try again later.
          <p>${error.message}</p>
        </div>
      `;
  }
